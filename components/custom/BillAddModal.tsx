			...
			<ModalBody className='px-4'>
				<div className='row g-4'>
					...
					<FormGroup id='Condition' label='Condition' className='col-md-3'>
						<>
						{['Power On', 'Display', 'Display Light', 'Touch Pad'].map((condition) => (
							<div key={condition} className='form-check'>
								<input
									...
									name='Condition'
									value={condition}
									checked={formik.values.Condition.toString().includes(condition)}
									onChange={(e) => {
					...
					<FormGroup id='Item' label='Accessories - from Customer' className='col-md-3'>
						<>
						{['Battery', 'Back Cover', 'Chager', 'SIM','SIM Card Tray','Memory Card','Memory Card Tray'].map((Item) => (
