...
export const getstockInByDate = async (date: string) => {
  ...
  console.log("Stock types:", data?.map(item => item.stock).filter((value, index, self) => self.indexOf(value) === index));
...
export const getAllStockRecords = async () => {
  ...
  console.log("Stock types present:", data?.map(item => item.stock).filter((v, i, a) => a.indexOf(v) === i));
